...
void PNMPCGEN::genCCodeBBOSQPCONSTCall()
{
 ...
 */
	...
	// Header guard
	HEADER_STREAM(C_HEADER_GUARD_BEGIN(FILE_SQP_BBOSQPCONSTCall_H_));
	HEADER_STREAM(INCLUDE(FILE_PNMPC_H_));

	// Function prototype for SQP - Constraints
	HEADER_STREAM(COMMENT("Function prototype for SQP - Constraints"));
	#if(ENABLE_CUDA)
		__CUDA_DEVICE_H__ 
	#endif
	HEADER_STREAM_T(TYPE_C << "func2BBOCONST_SQP(Real_C X_POINT[NMAX], const ParaStructC* XP_SQP)");
...
*/
	...
	// C code 
    // Include headers
	CCODE_STREAM(INCLUDE(FILE_SQP_BBOSQPCONSTCall_H_));
	CCODE_STREAM(INCLUDE(FILE_PNMPC_SQP_H_));
    
	// SQP function call - Constraints
	CCODE_STREAM(COMMENT("SQP function call - Constraints"));
	#if(ENABLE_CUDA)
		__CUDA_DEVICE_C__
	#endif
	CCODE_STREAM(TYPE_C << "func2BBOCONST_SQP(Real_C X_POINT[NMAX], const ParaStructC* XP_SQP)");
	...
	// Call to BBO Function - Constraints
	CCODE_STREAM(COMMENT("Call to BBO Function - Constraints"));
	CCODE_STREAM_NT("return BBO_FunctionCONST(X_SQP" << Paren[ParenOps::comma]);
	for (auto& item : db->accessAllInputs())
...
*/
	...
	// Export files
	EXPORT_CCODE(SRC_DIR,FILE_SQP_BBOSQPCONSTCall_C_);
	EXPORT_HEADER(INCLUDE_DIR,FILE_SQP_BBOSQPCONSTCall_H_);
}