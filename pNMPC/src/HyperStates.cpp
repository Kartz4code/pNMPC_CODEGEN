...
// Weighted norm 
/*HyperStates weightedNorm(const MATReal& Q, const TYPE& type, const int& normVal)
{
	HyperStates h_null, h_temp;
	switch (type)
	{
		case TYPE::STATE:
		{
			if ((Q.getNumColumns() == Q.getNumRows()) && (Q.getNumRows() == db->accessAllStates().size()) & (db->accessAllStates().size() == Q.getNumColumns()))
			{
				for (size_t i{ 0 };i < Q.getNumColumns(); i++)
				{
					for (size_t j{ 0 };j < Q.getNumRows(); j++)
						h_temp = h_temp + Q.getMatrixElement(i, j)*db->accessAllStates()[j]->getCurrentHyperState();

					h_null = h_null + ((h_temp)^(normVal));
					h_temp.flush();
				}
				return ((h_null)^(static_cast<Real>(1) / static_cast<Real>(normVal)));
			}
			else 
		...
		case TYPE::INPUT:
		{
			if (db->accessParameterizationMapEquations().size() == 0)
			{
				if ((Q.getNumColumns() == Q.getNumRows()) & (Q.getNumRows() == db->accessAllInputs().size()) & (db->accessAllInputs().size() == Q.getNumColumns()))
				{
					for (size_t i{ 0 };i < Q.getNumColumns(); i++)
					{
						for (size_t j{ 0 };j < Q.getNumRows(); j++)
							h_temp = h_temp + Q.getMatrixElement(i, j)*db->accessAllInputs()[j]->getCurrentHyperState();
			...
			else if (db->accessParameterizationMapEquations().size() != 0) 
			{
				if ((Q.getNumColumns() == Q.getNumRows()) & (Q.getNumRows() == db->accessParameterizationMapEquations().size()) & (db->accessParameterizationMapEquations().size() == Q.getNumColumns()))
				{
					for (size_t i{ 0 };i < Q.getNumColumns(); i++)
					{
						for (size_t j{ 0 };j < Q.getNumRows(); j++)
							h_temp = h_temp + Q.getMatrixElement(i, j)*(*db->accessParameterizationMapEquations()[j]);
		...
		case TYPE::PARAMETER:
		{
			if ((Q.getNumColumns() == Q.getNumRows()) & (Q.getNumRows() == db->accessAllParams().size()) & (db->accessAllParams().size() == Q.getNumColumns()))
			{
				for (size_t i{ 0 };i < Q.getNumColumns(); i++)
				{
					for (size_t j{ 0 };j < Q.getNumRows(); j++)
						h_temp = h_temp + Q.getMatrixElement(i, j)*db->accessAllParams()[j]->getCurrentHyperState();
